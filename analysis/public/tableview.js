...
async function visitTable(hexBase) {
  ...
  for (let i = 0; i <= 0xff; i++) {
    ...
    const list = optbl.filter(
      (k) =>
        k[hexBase.length] === subHex[0] && k[hexBase.length + 1] === subHex[1]
    );
    ...
    if (list.length == 1) {
      ...
      item.innerHTML +=
        "<div class='op-entry'>" + decoding.toUpperCase() + "</div>";
    } else {
...
let hexBase = "";
if (browseOffset != -1) {
  const suffix = window.location.href.substr(
    browseOffset + browseBaseUrl.length
  );
  for (
    let base = parseInt(suffix, 16);
    !isNaN(base) && base !== 0;
    base >>= 8
  ) {
